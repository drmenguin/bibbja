#!/bin/sh
# bibbja: Read the Word of God from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data bibbja.tsv | awk -v cmd=list "$(get_data bibbja.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data bibbja.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data bibbja.awk)" | ${PAGER}
	done
	exit 0
fi

get_data bibbja.tsv | awk -v cmd=ref -v ref="$*" "$(get_data bibbja.awk)" | ${PAGER}
